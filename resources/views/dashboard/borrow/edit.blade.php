            ...
            <div class="bg-white p-6 rounded-lg shadow">
                ...
                <form action="/dashboard/borrow/{{ $borrow->id }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $borrow->borrowed_at->format('d M Y') }}" disabled
                        ...
                        <input type="hidden" name="borrow_date" value="{{ $borrow->borrowed_at }}">
                        @error('borrow_date')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value=""></option>
                            <option value="panding" @selected(old('status', $borrow->status) == 'panding')>Diajukan</option>
                            <option value="borrowed" @selected(old('status', $borrow->status) == 'borrowed')>Dipinjam</option>
                            <option value="returned" @selected(old('status', $borrow->status) == 'returned')>
                                Dikembalikan</option>
                            <option value="rejected" @selected(old('status', $borrow->status) == 'rejected')>Ditolak</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
