    ...
    <div class="bg-white p-6 rounded-lg shadow">
      ...
      <form action="/dashboard/user/{{ $user->slug }}" method="POST" class="space-y-6">
        ...
        <div>
          ...
          <input type="password" name="password" id="password" value="{{$user->password ?? old('password') }}" required
        ...
        <div>
          ...
          <select name="role" id="role" required
            class="mt-1 p-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('role') border-red-500 @enderror">
