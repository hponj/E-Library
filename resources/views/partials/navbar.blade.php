<nav class="bg-gray-800" x-data="{ isOpen: false }">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="relative ml-3">
                    ...
                    <div 
                        ...
                        @if (auth()->user()->role == 'admin')
                            <a href="/dashboard" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Admin Panel</a>
                        @endif
                        
                        <form action="/logout" method="POST">
                            @csrf
                            <button type="submit" onclick="return confirm('Are you sure?')" class="block w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 grow text-left" role="menuitem" tabindex="-1" id="user-menu-item-2" >Sign out</button>
                        </form>
                    </div>
                </div>
                </div>
        ...
        @else
            <a href="/login" class="bg-indigo-600 hover:bg-indigo-500 text-white rounded-md px-3 py-2 text-sm font-semibold">Log in</a>
        @endauth
        </div>
    </div>
